...
def run(
  ...
  webcam = source.isnumeric() or source.endswith('.streams') or (is_url and
                                                                 not is_file)
...
with dai.Device(pipeline) as device:
  ...
  while True:
    ...
    for depthData in spatialData:
      ...
      cv2.putText(depthFrameColor,
                  f"X: {int(depthData.spatialCoordinates.x)} mm",
                  (xmin + 10, ymin + 20), fontType, 0.5, 255)
      cv2.putText(depthFrameColor,
                  f"Y: {int(depthData.spatialCoordinates.y)} mm",
                  (xmin + 10, ymin + 35), fontType, 0.5, 255)
      cv2.putText(depthFrameColor,
                  f"Z: {int(depthData.spatialCoordinates.z)} mm",
                  (xmin + 10, ymin + 50), fontType, 0.5, 255)
    # Show the frame
    cv2.imshow("depth", depthFrameColor)
    cv2.imshow("rgb", rgbFrame)
    run(parse_opt(), rgbFrame)
