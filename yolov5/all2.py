...
with dai.Device(pipeline) as device:
    ...
    while True:
        ...
        im = letterbox(rgbFrame, imgsz, stride=stride, auto=pt)[0]
        ...
        print(type(depthFrameColor), dir(depthFrameColor))
        quit()
        
        for j in range(len(outxywh[1:])):
            topLeft.x = outxywh[j][0]
            topLeft.y = outxywh[j][1]
            bottomRight.x = outxywh[j][2]
            bottomRight.y = outxywh[j][3]
